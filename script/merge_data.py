...
def merge_kitti_gpsimu(data_path, timestamps_file, output_path):
    ...
    processed_data = np.zeros([row_size, time_size + 9])
    for dir_path, subpaths, files in os.walk(data_path, False):
        ...
        for file in files:
            ...
            processed_data[count, time_size + 6:time_size +
                9] = per_data[11:14]
            count += 1
    np.savetxt(os.path.join(output_path, "./merge_data.txt"), processed_data, "%.10f", "\t");


if __name__ == "__main__":
    if (len(sys.argv) != 4):
        print("usage: python merge_data gpsimu_data_path timestamps_file")
        exit()
