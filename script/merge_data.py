...
def merge_kitti_gpsimu(data_path, timestamps_file, output_path):
    ...
    processed_data = np.zeros([row_size, time_size + 12])
    for dir_path, subpaths, files in os.walk(data_path, False):
        ...
        for file in files:
            ...
            count += 1
    np.savetxt(os.path.join(output_path, "./merge_data.txt"), processed_data,
               "%.10f", "\t")


if __name__ == "__main__":
    if (len(sys.argv) != 4):
        print("usage: python merge_data gpsimu_data_path timestamps_file output_path")
        exit()
